...
public class PoisonRegion extends RegionEffect{
    public final static TextureRegion TEXTURE_REGION = new TextureRegion(new Texture("effect/poison_region.png"));
    ...
    public PoisonRegion(SimpleCharacter owner, float x, float y) {
        this.owner = owner;
        this.x = x;
        this.y = y;
    }
    ...
    public void update(float deltaTime, WorldMap map) {
            ...
            for (Room room : map.getRooms()) {
                for (Monster monster : room.getMonsterAlive()) {
                    ...
                    if (centerPos.dst(centerMonster) <= radius) {
                        ...
                        monster.getHit(damage, DamageType.POISON);
                    }
                }
            }
        }
        if (owner == null || owner instanceof Monster) {
            ...
            if (centerPos.dst(playerPos) <= radius) {
                ...
                player.getHit(damage, DamageType.POISON);
            }
        }
    }
